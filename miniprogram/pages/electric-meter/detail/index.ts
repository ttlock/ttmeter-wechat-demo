// pages/electric-meter/detail/index.ts
...
import * as ElectricMeterUserAPI from "../../../api/interfaces/electricMeterUser";
...
Page({
    ...
    handleCheckInput(event) {
        ...
        if (target == "MAX_POWER") {
            const maxPower = Number(value?.maxPower || 0);
            if (!(maxPower > 0 && maxPower <= 255)) {
                HttpHandler.showErrorMsg("请输入正确的功率阈值(取值范围1-255)");
                return false;
            } else {
                return true;
            }
        ...
        } else if (target === "REMAINDER_KWH") {
            if (!/^[1-9]{1}[0-9]{0,4}$/.test(value?.remainderKwh || "")) {
                HttpHandler.showErrorMsg("请输入正确的剩余电量(1-99999)");
                return false;
            } else {
                return true;
            }
        } else if (target == "RECHARGE_KWH") {
            if (!/^[1-9]{1}[0-9]{0,2}$/.test(value?.rechargeKwh || "")) {
                HttpHandler.showErrorMsg("请输入正确的待充值电量(1-999)");
                return false;
            } else {
                return true;
            }
    ...
    _handleSubmit(event) {
        ...
        switch(target) {
        case "MAX_POWER": this.setMaxPower(Number(value?.maxPower)); break;
    ...
    _handleLoadAdminInfo(meterId: number, needInit?: boolean) {
        ...
        ElectricMeterAPI.detail({ electricMeterId: meterId }).then(res => {
            wx.hideLoading();
            if (HttpHandler.isResponseTrue(res)) {
                if (res?.electricMeterId) {
    ...
    _handleLoadTenantInfo(meterMAC: string, needInit?: boolean) {
        ...
        wx.removeStorageSync("access_token");
        wx.removeStorageSync("uid");
        ElectricMeterUserAPI.detail({ mac: meterMAC }).then(res => {
            wx.hideLoading();
            if (HttpHandler.isResponseTrue(res)) {
                if (res?.electricMeterId) {
                    this.setData({ meterInfo: res });
    ...
    _handleInit() {
		...
		const flag = plugin.setClientParam(JSON.parse(JSON.stringify({
			url: `${Config.BASE_DOMAIN}/v3/electricMeter/executeCommand`, // 服务器域名(必填)
	...
	async DeleteMeter() {
		...
		wx.showLoading({ title: "Loading..." });
		this.setData({ errMsg: `正在删除蓝牙电表：${MAC}` });
		try {
			const connRes = await TTElectricMeter.connect(MAC);
			if (connRes?.errCode != 0) throw(connRes);
			/**
			 * @TTMeterPlugin 删除蓝牙智能电表(0.0.3)
			 */
			const res = await TTElectricMeter.delete();
			wx.hideLoading({});
			console.log("删除蓝牙电表数据完成", res);
            this.setData({ errCode: res?.errCode, errMsg: `删除蓝牙电表数据结束：${res?.errMsg}` });
            if (res?.errCode == 0) {
    ...
    /** 蓝牙电表抄表(0.0.3) */
    async syncElecMeterState() {
        const meterInfo = this.data?.meterInfo as IElectricMeter.Result.Detail | IElectricMeterUser.Result.Detail;
        const MAC = meterInfo?.mac;
        const plugin = requirePlugin("ttmeter-plugin") as TTMeterPlugin; // 引入插件
        const TTElectricMeter = plugin.TTElectricMeter;
		wx.showLoading({ title: "Loading..." });
		...
		try {
            ...
            if (connRes?.errCode != 0) throw(connRes);
			/**
			 * @TTMeterPlugin 蓝牙电表抄表，与服务器同步蓝牙电表数据(0.0.3)
			 * @description 调用成功后，请通过服务器接口重新获取蓝牙电表数据
			 */
			const res = await TTElectricMeter.readData();
			// wx.hideLoading({});
    ...
    _toChargeMeter() {
        const ID = this.data.meterId;
        const MAC = this.data.MAC;
        if (!!ID) this.setData({ showInput: 4, rechargeKwh: "" }); // 管理员充值
        else if (!!MAC) this.setData({ showInput: 2, executeToken: "" }); // 租客登录
    },
    /** 蓝牙电表充值(0.0.3) */
    async chargeElecMeter(tokenOrKwh: string | number) {
        const meterInfo = this.data?.meterInfo as IElectricMeter.Result.Detail;
        const MAC = meterInfo?.mac;
        const plugin = requirePlugin("ttmeter-plugin") as TTMeterPlugin; // 引入插件
        const TTElectricMeter = plugin.TTElectricMeter;
        const executeToken = typeof tokenOrKwh == "string" ? tokenOrKwh : undefined;
        const rechargeKwh = typeof tokenOrKwh == "string" ? undefined : tokenOrKwh;
        const rechargeAmount = typeof tokenOrKwh == "string" ? undefined : (tokenOrKwh * Number(meterInfo?.price || 0));
		wx.showLoading({ title: "Loading..." });
		...
		try {
			...
			if (connRes?.errCode != 0) throw(connRes);
			/**
			 * @TTMeterPlugin 蓝牙电表充值(0.0.3)
			 * @description 调用成功后，请【额外】调用【蓝牙抄表接口】更新电表数据
			 */
			let res = await TTElectricMeter.recharge(JSON.parse(JSON.stringify({
				rechargeAmount: rechargeAmount || undefined,
				rechargeKwh: rechargeKwh || undefined,
				executeToken: executeToken || undefined // TODO 充值token(必填)
			})));
			console.log("蓝牙电表充值完成", res);
			this.setData({ errCode: res?.errCode, errMsg: `蓝牙电表充值结束：${res?.errMsg}, 正在抄表`, showInput: false });
			/**
			 * @TTMeterPlugin 蓝牙电表抄表，与服务器同步蓝牙电表数据(0.0.3)
			 * @description 调用成功后，请通过服务器接口重新获取蓝牙电表数据
			 */
			res = await TTElectricMeter.readData();
			// wx.hideLoading({});
			console.log("同步蓝牙电表数据完成", res);
            this.setData({ errCode: res?.errCode, errMsg: `同步蓝牙电表数据结束：${res?.errMsg}` });
            this.handleReload();
    ...
    /** 设置剩余电量 */
    _toSetElecMeter() {
        this.setData({ showInput: 3, remainderKwh: "" });
    },
    /** 蓝牙电表设置电量(0.0.3) */
	async setElecMeter(remainderKwh: number) {
        const meterInfo = this.data?.meterInfo as IElectricMeter.Result.Detail;
        const MAC = meterInfo?.mac;
        const plugin = requirePlugin("ttmeter-plugin") as TTMeterPlugin; // 引入插件
        const TTElectricMeter = plugin.TTElectricMeter;
		wx.showLoading({ title: "Loading..." });
	    this.setData({ errMsg: `正在设置电表电量: ${remainderKwh}kwh` });
		try {
			const connRes = await TTElectricMeter.connect(MAC);
			if (connRes?.errCode != 0) throw(connRes);
			/**
			 * @TTMeterPlugin 设置蓝牙电表电量(0.0.3)
			 * @description 调用成功后，请【额外】调用【蓝牙抄表接口】更新电表数据
			 */
			let res = await TTElectricMeter.setRemainingElectricity({
				remainderKwh: remainderKwh, // 剩余电量(必填)
			});
			console.log("电量设置完成", res);
            this.setData({ errCode: res?.errCode, errMsg: `电量设置结束：${res?.errMsg}, 正在抄表中`, showInput: false });
            /**
			 * @TTMeterPlugin 蓝牙电表抄表，与服务器同步蓝牙电表数据(0.0.3)
			 * @description 调用成功后，请通过服务器接口重新获取蓝牙电表数据
			 */
            res = await TTElectricMeter.readData();
            this.setData({ errCode: res?.errCode, errMsg: `抄表成功` });
	        this.handleReload();
	    } catch(err) {
			wx.hideLoading({});
			this.setData({ errCode: err?.errCode, errMsg: `操作结束：${err?.errMsg}` });
	...
	/** 蓝牙电表清空电量(0.0.3) */
	async clearElecMeter() {
	    const meterInfo = this.data?.meterInfo as IElectricMeter.Result.Detail;
        const MAC = meterInfo?.mac;
        const plugin = requirePlugin("ttmeter-plugin") as TTMeterPlugin; // 引入插件
        const TTElectricMeter = plugin.TTElectricMeter;
		wx.showLoading({ title: "Loading..." });
	    this.setData({ errMsg: "正在清空电表电量" });
		try {
			const connRes = await TTElectricMeter.connect(MAC);
			if (connRes?.errCode != 0) throw(connRes);
			/**
			 * @TTMeterPlugin 清空蓝牙电表电量(0.0.3)
			 * @description 调用成功后，请【额外】调用【蓝牙抄表接口】更新电表数据
			 */
			let res = await TTElectricMeter.clearRemainingElectricity();
			console.log("电量清空完成", res);
			this.setData({ errCode: res?.errCode, errMsg: `电量清空结束：${res?.errMsg}`, showInput: false });
	        /**
			 * @TTMeterPlugin 蓝牙电表抄表，与服务器同步蓝牙电表数据(0.0.3)
			 * @description 调用成功后，请通过服务器接口重新获取蓝牙电表数据
			 */
	...
	async setPowerOnOff() {
        ...
        const TTElectricMeter = plugin.TTElectricMeter;
		wx.showLoading({ title: "Loading..." });
		this.setData({ errMsg: `正在设置通断电状态：${meterInfo?.onOff == 1 ? '断电': "通电"}` });
		try {
			const connRes = await TTElectricMeter.connect(MAC);
			if (connRes?.errCode != 0) throw(connRes);
			/**
			 * @TTMeterPlugin 设置电表通断电状态(0.0.3)
			 * @description 若调用时设备已断开连接，该接口将跳过操作直接返回错误码
			 */
			const res = await TTElectricMeter.setPowerOnOff({
				powerOn: meterInfo?.onOff == 1 ? false : true
			});
			console.log("设置电表通断电状态完成", res);
	        this.setData({ errCode: res?.errCode, errMsg: `设置电表通断电状态结束：${res?.errMsg}` });
            this.handleReload();
	    } catch(err) {
			this.setData({ errCode: err?.errCode, errMsg: `操作结束：${err?.errMsg}` });
		} finally {
	...
	async configWorkMode() {
        ...
        const TTElectricMeter = plugin.TTElectricMeter;
		wx.showLoading({ title: "Loading..." });
		this.setData({ errMsg: `正在设置电表付费模式：${meterInfo?.payMode == 0 ? '预付费模式': '计能模式'}` });
		try {
			const connRes = await TTElectricMeter.connect(MAC);
			if (connRes?.errCode != 0) throw(connRes);
			/**
			 * @TTMeterPlugin 设置电表付费模式(0.0.3)
			 * @description 调用成功后，请通过服务器接口重新获取蓝牙电表数据
			 */
			const res = await TTElectricMeter.setPayMode({
				payMode: meterInfo?.payMode == 0 ? 1 : 0,
				price: Number(meterInfo?.price)
			});
			console.log("设置电表付费模式完成", res);
            this.setData({ errCode: res?.errCode, errMsg: `设置电表付费模式结束：${res?.errMsg}` });
			this.handleReload();
        } catch(err) {
			this.setData({ errCode: err?.errCode, errMsg: `操作结束：${err?.errMsg}` });
		} finally {
    ...
    /** 设置电表最大功率 */
    _toSetMaxPower() {
        this.setData({ showInput: 1, maxPower: "" });
    },
    /** 设置电表最大功率(0.0.3) */
	async setMaxPower(maxPower: number) {
		const meterInfo = this.data?.meterInfo as IElectricMeter.Result.Detail;
        const MAC = meterInfo?.mac;
        const plugin = requirePlugin("ttmeter-plugin") as TTMeterPlugin; // 引入插件
        const TTElectricMeter = plugin.TTElectricMeter;
		wx.showLoading({ title: "Loading..." });
		this.setData({ errMsg: `正在设置电表最大功率: ${maxPower}kw` });
		try {
			const connRes = await TTElectricMeter.connect(MAC);
			if (connRes?.errCode != 0) throw(connRes);
			/**
			 * @TTMeterPlugin 设置电表最大功率(0.0.3)
			 * @description 调用成功后，请通过服务器接口重新获取蓝牙电表数据
			 */
			const res = await TTElectricMeter.setMaxPower({
				maxPower: maxPower,
			});
			console.log("设置电表最大功率完成", res);
            this.setData({ errCode: res?.errCode, errMsg: `设置电表最大功率结束：${res?.errMsg}, 当前最大功率阈值：${maxPower}kw`, showInput: false });
            this.handleReload();
	    } catch(err) {
			this.setData({ errCode: err?.errCode, errMsg: `操作结束：${err?.errMsg}` });
		} finally {
	...
	async getFeatureValue() {
        ...
        const TTElectricMeter = plugin.TTElectricMeter;
		wx.showLoading({ title: "Loading..." });
		this.setData({ errMsg: "正在查询电表特征值" });
		try {
			const connRes = await TTElectricMeter.connect(MAC);
			if (connRes?.errCode != 0) throw(connRes);
			/**
			 * @TTMeterPlugin 获取电表特征值(0.0.3)
			 * @description 调用成功后，请通过服务器接口重新获取蓝牙电表数据
			 */
			const res = await TTElectricMeter.getFeatureValue({});
			console.log("查询电表特征值完成", res);
            this.setData({ errCode: res?.errCode, errMsg: `获取电表特征值结束：${res?.errMsg}` });
            this.handleReload();
	    } catch(err) {
			this.setData({ errCode: err?.errCode, errMsg: `操作结束：${err?.errMsg}` });
		} finally {
