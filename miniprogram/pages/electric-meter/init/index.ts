// pages/electric-meter/init/index.ts
...
Page({
    ...
    handleCheckInput(event) {
        ...
        if (!/^[1-9]{1}[0-9]{0,7}$/.test(value?.price || "")) {
            HttpHandler.showErrorMsg("请输入正确的电费单价(1-99999999)");
            return false;
        } else {
            return true;
        }
	...
	startScan: async function() {
		...
		this.setData({ errMsg: "正在开启蓝牙电表扫描" });
		/**
		 * @TTMeterPlugin 开启蓝牙电表扫描(0.0.3)
		...
		TTElectricMeter.startScan({
			...
			wx.hideLoading({});
			console.log("开启蓝牙电表扫描完成", res);
			this.setData({ errCode: res?.errCode, errMsg: `开启蓝牙电表扫描结束：${res?.errMsg}` });
		})
	},
	/** 关闭蓝牙电表扫描(0.0.3) */
	stopScan: async function() {
        const plugin = requirePlugin("ttmeter-plugin") as TTMeterPlugin; // 引入插件
		wx.showLoading({ title: "" });
		this.setData({ errMsg: "关闭蓝牙电表扫描" });
		/**
		 * @TTMeterPlugin 关闭蓝牙电表扫描(0.0.3)
		 */
		plugin.stopScan().then(res => {
			wx.hideLoading({});
			console.log("关闭蓝牙电表扫描完成", res);
			this.setData({ errCode: res?.errCode, errMsg: `关闭蓝牙电表扫描结束：${res?.errMsg}`, deviceList: [] });
		})
    },
    /** 点击添加蓝牙智能电表 */
    toInit: async function (event) {
        const plugin = requirePlugin("ttmeter-plugin") as TTMeterPlugin; // 引入插件
        const deviceInfo = JSON.parse(JSON.stringify(event?.currentTarget?.dataset?.item || "{}")) as TTElectricMeter.DeviceModel;
		if (typeof deviceInfo?.isInited != "boolean") return;
		else if (deviceInfo.isInited) return wx.showModal({ content: '蓝牙电表已被添加，请先删除', showCancel: false });
		try {
			wx.showLoading({ title: "" });
			this.setData({ errMsg: "正在关闭蓝牙电表扫描" });
			/**
			 * @TTMeterPlugin 停止蓝牙电表扫描(0.0.3)
			 * @description 安卓设备需【额外】开启【位置开关】、微信【位置授权】
			 */
			const stopRes = await plugin.stopScan();
            console.log("停止蓝牙电表扫描完成", stopRes);
            this.setData({
                errCode: stopRes?.errCode,
                errMsg: `停止蓝牙电表扫描结束：${stopRes?.errMsg}`,
	...
	add: async function(price: number) {
        const plugin = requirePlugin("ttmeter-plugin") as TTMeterPlugin; // 引入插件
        const TTElectricMeter = plugin.TTElectricMeter;
		const deviceInfo = this.data?.deviceInfo as TTElectricMeter.DeviceModel;
		if (typeof deviceInfo?.isInited != "boolean") return;
		else if (deviceInfo.isInited) return wx.showModal({ content: '蓝牙电表已被添加，请先删除', showCancel: false });
		try {
            wx.showLoading({ title: "" });
			this.setData({ errMsg: `正在连接智能电表：${deviceInfo.mac}`, deviceList: [] });
			const connRes = await TTElectricMeter.connect(deviceInfo.mac);
			...
			console.log("电表连接", deviceInfo, 1111)
			this.setData({ errMsg: `正在添加蓝牙电表${deviceInfo?.name}, 单价：${price}元/kwh, 付费方式：${deviceInfo?.payMode == 1 ? '预付费' : '计能'}`});
			const addRes = await TTElectricMeter.add({
				name: deviceInfo?.name,
				payMode: deviceInfo?.payMode,
				price: price
			});
			console.log("添加蓝牙电表完成", addRes);
			this.setData({
                errCode: addRes?.errCode,
                errMsg: `添加蓝牙电表结束：${addRes?.errMsg}`,
                showInput: addRes?.errCode == 0 ? false : true
            });
            if (addRes?.errCode == 0) {
                HttpHandler.showErrorMsg("蓝牙电表已添加");
